...
public class RSAEncrypt {
     ...
     */
    public static Charset charset = JSONConfig.DEFAULT_CHARSET;
    /**
    ...
    public static void generateKeyPair (String[] pairMap) {
        try {
            ...
            pairMap[0] = new String(Base64.getEncoder().encode(privateKey.getEncoded()), charset);
            pairMap[1] = new String(Base64.getEncoder().encode(publicKey.getEncoded()), charset);
    ...
    public static String encrypt (String info, String pubilcKeyString) {
        return encrypt(info.getBytes(charset), pubilcKeyString);
    }
    /**
    ...
    public static String encrypt (byte[] info, String pubilcKeyString) {
        try {
            byte[] publicKeyBytes = Base64.getDecoder().decode(pubilcKeyString.getBytes(charset));
            ...
            return new String(Base64.getEncoder().encode(cipher.doFinal(info)), charset);
    ...
    public static String decryptToString (String encryptinfo, String privateKeyString) {
        return new String(decryptToBytes(encryptinfo, privateKeyString), charset);
    }
    /**
    ...
    public static byte[] decryptToBytes(String encryptinfo, String privateKeyString) {
        try {
            byte[] privateKeyBytes = Base64.getDecoder().decode(privateKeyString.getBytes(charset));
            ...
            return cipher.doFinal(Base64.getDecoder().decode(encryptinfo));
